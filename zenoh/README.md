# CAMPUS Edge–Device Communication Testbed (Zenoh Implementation)

This directory contains the **Zenoh protocol implementation** for the CAMPUS (Cooperative Aggregation & Mapping for Perception in Ubiquitous Sensing) experiment testbed. It is a sub-module of the ENVELOPE project under EU Horizon Europe.
...
This testbed **does not** implement perception or mapping logic. Instead, it isolates the communication segment so we can evaluate latency, reliability, and selective downlink behaviour under different protocol stacks (gRPC, Zenoh, and later MQTT-over-QUIC).
...
graph TD
    ...
    EN -- Publishes campus/cmd/device-1 and campus/cmd/device-2 --> ZR
    ZR -- Routes to Device-1 --> D1
    ZR -- Routes to Device-2 --> D2
    D1 -- Publishes campus/ack/device-1 --> ZR
    D2 -- Publishes campus/ack/device-2 --> ZR
    ZR -- Routes campus/ack/** samples --> EN
    D3 -. connected but not targeted .- ZR
```
...
* **Zenoh Router:** Routing component (`zenohd`) that forwards matching pub/sub samples between publishers and subscribers.
* **Devices (Far Edge / Vehicles):** Client processes that subscribe to their own command key (`campus/cmd/<device-id>`) to receive updates, and publish acknowledgments to `campus/ack/<device-id>`.
* **Idle Devices:** Devices (like `device-3` in the diagram) that connect to the Zenoh router but do not receive any downlink commands. These are explicitly included in the testbed to emulate a connected vehicle that remains in the network but is excluded from selected downlink to model relevance filtering.
...
├── src/
│   ├── edge_zenoh.py         # Main edge controller & statistical logging script
│   └── device_zenoh.py       # Main device client simulator script
├── scripts/
│   └── run_devices.py        # Host-level local multi-device launcher script
├── results/
│   └── results_zenoh_*.csv   # Generated RTT metric files (synced from Docker)
...
* **[Implemented] Containerized Topology (Docker Compose):** Full router, edge, and device network setup.
...
* **[Planned] Network Delay & Packet Loss Simulation:** Emulating realistic wireless link conditions.
* **[Planned] MQTT-over-QUIC Protocol implementation:** For performance comparison.

---

## 5. Getting Started

Make sure you have Python 3 and Zenoh installed (if running locally), or Docker & Docker Compose (if running containerized).

### Method A: Automated Experiment Harness (Recommended)

This method uses one script to automatically verify/start the Zenoh router in Docker, spawn simulated devices in the background, run the foreground edge experiment node for a set duration, and clean up everything when finished.

#### On Windows (PowerShell):
```powershell
.\exp_baseline_zenoh.ps1 -Devices 5 -DurationSec 10 -IntervalSec 0.2 -OutputCsv results/zenoh_test.csv
```

#### On Linux / macOS / Git Bash:
```bash
./exp_baseline_zenoh.sh 5 100 0.2 10 results/zenoh_test.csv
# Syntax: ./exp_baseline_zenoh.sh [devices] [payload_bytes] [interval_sec] [duration_sec] [output_csv]
```
...
1. **Start your local Zenoh Router:**
   ```bash
   zenohd
   ```
...
3. **Start the Edge node with custom options:**
   ...
   ```bash
   python src/edge_zenoh.py --devices 10 --duration 60 --payload-size 100 --interval 0.1 --output results/my_run.csv
   ```
4. **Termination & Cleanup Lifecycle:** 
   * **Edge Node:** Press `Ctrl + C` in the edge terminal. It intercepts the signal to write the RTT latency CSV file to your local directory and closes its Zenoh session.
   ...
   * **Router:** The local `zenohd` process can be stopped separately (e.g. via `Ctrl + C` in its own window).
...
|---|---|---|---|
| `--router` | `ZENOH_ROUTER` | `tcp/localhost:7447` | Zenoh router endpoint address |
...
* **No multi-edge coordination yet:** Only one edge controller and one Zenoh router are in scope.
