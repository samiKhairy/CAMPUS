...
At $N=20$ devices, 10~Hz, 30~s, the edge sends 6,000 commands and receives 6,000 acknowledgements --- 12,000 application-level messages per run. For broker-based protocols (MQTT), this becomes \textbf{24,000 broker message transitions}.

At $N=50$, the same calculation gives 15,000 commands and 15,000 acknowledgements --- 30,000 application-level messages per run, or \textbf{60,000 broker message transitions}. This is the load that exposes the architectural limits of each approach.
...
Figure~\ref{fig:n20_comparison} shows the same data as a side-by-side snapshot at $N=20$, with exact p50 values annotated and p95 shown as error bars. The degraded panel uses a log scale so the MQTT TCP bar (19.9~s) and the other protocols ($\sim$160--183~ms) are both visible.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{results/unified/protocol_comparison_N20.png}
\caption{Protocol comparison at $N=20$, 100~B, 10~Hz. Bars show median RTT (p50); error bars show p95. Left: clean LAN. Centre: normal 5G. Right: degraded 5G (log scale). gRPC loss labels are shown in red inside the bars. MQTT TCP is 19.9~s median under degraded 5G while all other protocols stay below 200~ms.}
...
\caption{Median RTT vs device count under degraded 5G (100~B, 10~Hz, log scale). MQTT TCP stays near the $\sim$350~ms floor through $N=5$, then collapses --- 2.4~s at $N=10$ and 19.9~s at $N=20$. All other protocols stay flat between 160~ms and 250~ms across $N=1$ to $N=20$.}
...
\begin{itemize}
    ...
    \item gRPC here is still HTTP/2 over TCP; I did not test gRPC over HTTP/3 (QUIC). The QUIC transport benefit is already isolated in these results through the Zenoh-TCP$\to$Zenoh-QUIC and MQTT-TCP$\to$MQTT-QUIC comparisons (same protocol, transport swapped), so a third QUIC stack would mainly re-confirm the same mechanism. It is also non-trivial: mainstream gRPC has no native HTTP/3 support and would require an experimental build or a QUIC-terminating proxy. I'd hold gRPC-over-HTTP/3 as a focused transport-only study for later, rather than expand the current matrix and mix the transport and architecture variables this design keeps separate.

    \item At $N=50$, several Zenoh and Zenoh-QUIC runs returned no data (header-only CSV files). The root cause was not isolated --- likely WSL2 disk exhaustion or a concurrency race condition in the test harness, rather than a Zenoh protocol limit, since the failure pattern was inconsistent across runs and container resource usage was not profiled. The $N \leq 20$ data is clean and sufficient for the architecture decision; $N=50$ should be revisited on the physical testbed.
\end{itemize}
